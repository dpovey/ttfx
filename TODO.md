...
- **typemacro** transforms code at build time (source-to-source)
...
- typemacro works within the TypeScript compiler itself
...
- [x] **IDE Plugin / Language Service** -- TS language service plugin at `typemacro/language-service`. Suppresses false-positive diagnostics from macro decorators and implicit extension methods, provides completions inside `@derive()` and for extension methods on member access, and shows macro/extension info on hover. Configure via `"plugins": [{ "name": "typemacro/language-service" }]` in tsconfig.json.
- [x] **VSCode Extension** -- Full IDE extension at `packages/vscode-typemacro/` (`@typemacro/vscode`). Manifest-driven architecture: all macro names come from `typemacro.manifest.json` (generated by `typemacro build --manifest`), so adding new macros automatically gets IDE support. Features:
  ...
  - **Manifest CLI**: `typemacro build --manifest` serializes the global registry to `typemacro.manifest.json`. Extension watches the file and hot-reloads.
...
- [x] **Pre-processor mode** -- unplugin-based integration works with Vite, Webpack, esbuild, Rollup, and Rspack without patching tsc. Import from `typemacro/vite`, `typemacro/webpack`, etc.
...
- [ ] **Documentation site** -- No docs beyond DESIGN.md and README. Need a proper docs site with examples, API reference, and guides for writing custom macros.

### Medium Priority

- [x] **Consolidate src/ and packages/ directories** — Code duplication between legacy `src/` and `packages/` addressed:
  - **packages/ now builds cleanly**: All `@typemacro/*` packages build successfully with full DTS generation
...
- [x] **Test coverage gaps** — Added comprehensive tests:
  ...
  - Remaining gaps: `packages/comptime/` edge cases, `packages/cats/src/effect/` Effect types, `packages/vscode-typemacro/` extension tests, unplugin adapter integration tests

- [ ] **Modular macro registration** — Currently all macros are eagerly registered at transformer load time via side-effect imports in `src/macros/index.ts`. Refactor to:
  - Split into "core" macros (always loaded: comptime, derive, operators, reflect) and "module" macros (lazy-loaded)
  - Replace `src/use-cases/` with proper supported modules (e.g., `typemacro/testing`, `typemacro/sql`, `typemacro/comprehensions`)
  - Add import detection in transformer to lazy-load only macros from detected imports
  - Each module package should self-register when imported
  - Support `macroModules` config for external/third-party macro packages
  - **Implementation**: Scan source file imports before registration, dynamically import macro registration modules based on detected `typemacro/*` imports
  - **References**: `src/macros/index.ts`, `src/core/registry.ts`, `src/transforms/macro-transformer.ts` (line 32 eager import)

- [x] **Import-scoped macro activation** -- Macros are now only activated when the user imports the placeholder from the declaring module (e.g., `import { comptime } from "typemacro"`). The transformer uses `getAliasedSymbol` to trace identifiers through barrel re-exports, renamed imports, and namespace imports back to their origin module. Macros declare their source via the `module` field on `MacroDefinitionBase`. Macros without `module` still work via name-based lookup (backward compatibility). All built-in macros now declare `module: "typemacro"`.
...
- [x] **Watch mode integration** -- `npx typemacro watch` is implemented in the CLI.
- [x] **CLI tool** -- `npx typemacro build/watch/check` commands are implemented. See `src/cli/index.ts`.
...
- [ ] **Specialization Ergonomics** -- Three levels of compile-time specialization for typeclass-based generic programming:
  ...
  **Level 2: Scoped Specialization** (design for later)
  ...
  import { optionMonad } from "@typemacro/cats" with { implicit: true };
  ```
...
- [ ] **`Eval` monad for cats library** -- Add a trampolined lazy evaluation monad (`Eval<A>`) to `@typemacro/cats`. **Not needed for List operations** (those were converted to iterative while loops), but valuable for:
...
- [ ] **Macro hygiene improvements** -- Generated identifiers use `__typemacro_` prefix, but there's no formal hygiene system preventing name collisions in complex expansions.
...
All in `src/use-cases/type-system/`. Tests in `tests/type-system.test.ts` (80 tests).
...
- Typeclasses (`Functor`, `Monad`, etc.) moved to `@typemacro/cats` package
...
3. The `@typemacro/zero-cost` `match` macro already handles discriminated union matching better
...
`src/use-cases/comprehensions/`. Tests in `tests/comprehensions.test.ts` (51 tests).
...
```typescript
import { quote, unquote } from "typemacro/quote";
...
```typescript
import { defineSyntaxMacro } from "typemacro";
...
```typescript
import { defineDeriveMacro, quote } from "typemacro";
...
```typescript
import { includeStr, includeBytes, includeJson } from "typemacro";
...
```typescript
import { cfg, cfgAttr } from "typemacro";
...
- [ ] **Configuration source** — Read conditions from:
  - `typemacro.config.ts` / `typemacro.config.json` (static config)
...
```typescript
import { collectTypes, moduleIndex } from "typemacro";
...
**Problem**: Generated identifiers use `__typemacro_` prefix convention, but there's no formal system preventing capture. Complex macro compositions can collide.
...
const internal = ctx.generateUniqueName("temp"); // Gets "__typemacro_temp_42" (already exists)
...
ctx.withHygieneScope(() => {
  ...
  const a = ctx.createIdentifier("x"); // Actually __typemacro_x_scope7_0
  const b = ctx.createIdentifier("x"); // Same: __typemacro_x_scope7_0 (same scope, same name)
});
```
...
npx typemacro expand src/models/user.ts

# Show expanded output for a specific macro call (by line number)
npx typemacro expand src/models/user.ts:42

# Diff mode: show original vs expanded side-by-side
npx typemacro expand --diff src/models/user.ts

# JSON AST output (for tooling)
npx typemacro expand --ast src/models/user.ts
```
...
- [ ] **Content-hash cache** — Hash macro inputs (source text + type info + config) → cache expanded output:
  ...
  - Storage: `.typemacro-cache/` directory (gitignored)
...
- [ ] **Cache warming** — `npx typemacro cache-warm` pre-populates cache for CI

- [ ] **Cache stats** — `npx typemacro cache-stats` shows hit rate, size, staleness
...
- [ ] **`typemacro/eslint-plugin`** package with rules:
...
- [ ] **Monorepo publish** — Publish all packages: `typemacro`, `@typemacro/core`, `@typemacro/cats`, etc.
