...
function readTsConfig(configPath) {
  ...
  const configFile = ts7.readConfigFile(absolutePath, ts7.sys.readFile);
  if (configFile.error) {
    const message = ts7.flattenDiagnosticMessageText(
  ...
  if (parsed.errors.length > 0) {
    const messages = parsed.errors.map(
      (d) => ts7.flattenDiagnosticMessageText(d.messageText, "\n")
    );
...
function reportDiagnostics(diagnostics) {
  ...
  for (const diagnostic of diagnostics) {
    ...
    if (diagnostic.file && diagnostic.start !== void 0) {
      ...
      const prefix = diagnostic.category === ts7.DiagnosticCategory.Error ? "error" : "warning";
      console.error(
    ...
    if (diagnostic.category === ts7.DiagnosticCategory.Error) {
      errorCount++;
    }
  }
  return errorCount;
...
function build(options) {
  ...
  if (options.verbose) {
    ...
    console.log(
      `[typemacro] Compiling ${config.fileNames.length} files...`
    );
  }
  ...
  const program = ts7.createProgram(config.fileNames, compilerOptions);
  ...
  const allDiagnostics = ts7.getPreEmitDiagnostics(program).concat(emitResult.diagnostics);
...
[typemacro] Found ${errorCount} error${errorCount === 1 ? "" : "s"} in ${fileCount} files.`
      ...
      console.log(
        `[typemacro] Successfully compiled ${fileCount} files.`
      );
    }
  }
...
function watch(options) {
  const configPath = ts7.findConfigFile(
  ...
  const host = ts7.createWatchCompilerHost(
    ...
    {},
    ts7.sys,
    ts7.createEmitAndSemanticDiagnosticsBuilderProgram,
    (diagnostic) => {
