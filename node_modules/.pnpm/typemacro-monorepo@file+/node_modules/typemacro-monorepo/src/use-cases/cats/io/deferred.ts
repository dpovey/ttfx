...
export class Deferred<A> {
  ...
  get(): IO<A> {
    return IO.async<A>((cb) => {
      if (this._value._tag === "Some") {
        cb(Right(this._value.value));
      } else {
  ...
  isCompleted(): IO<boolean> {
    return IO.delay(() => this._value._tag === "Some");
  }
...
export class MVar<A> {
   ...
   */
  isEmpty(): IO<boolean> {
    return IO.delay(() => this._value._tag === "None");
  }
