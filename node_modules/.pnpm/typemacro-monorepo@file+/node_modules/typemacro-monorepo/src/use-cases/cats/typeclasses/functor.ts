...
/**
 * Functor for 2-arity type constructors (like Either<E, A>)
 */
...
export function tupleLeft<F>(
  F: Functor<F>,
): <A, B>(fa: Kind<F, A>, b: B) => Kind<F, [B, A]> {
...
export function tupleRight<F>(
  F: Functor<F>,
): <A, B>(fa: Kind<F, A>, b: B) => Kind<F, [A, B]> {
...
export function flap<F>(
  F: Functor<F>,
): <A, B>(a: A, fab: Kind<F, (a: A) => B>) => Kind<F, B> {
...
/**
 * Create a Functor2 instance from a map function
 */
export function makeFunctor2<F>(
  URI: F,
...
export function composeFunctor<F, G>(
  ...
  return {
    URI: [F.URI, G.URI] as [F, G],
    map: (fga, f) => F.map(fga, (ga) => G.map(ga as Kind<G, unknown>, f)),
  };
}
