...
export function fromOption<E, A>(
  ...
  return isSome(opt) ? Valid(opt.value) : Invalid(onNone());
}
...
export function ensure<E, A>(
  ...
  return Invalid(S.combine(v.error as E, onFalse(v.value)));
}
...
export const Validated = {
  ...
  mapN: <E, A extends unknown[], B>(
    ...
    const f = args[args.length - 1] as (...args: A) => B;
    const vs = args.slice(0, -1) as { [K in keyof A]: ValidatedNel<E, A[K]> };

    if (vs.length === 0) {
      return Valid(f(...([] as unknown as A)));
    }
    if (vs.length === 1) {
      return map(vs[0], (a) => f(a as unknown as A[0]) as B) as ValidatedNel<
        E,
        B
      >;
    }
    if (vs.length === 2) {
      return map2Nel(vs[0], vs[1], (a, b) => f(a, b) as B) as ValidatedNel<
        E,
        B
      >;
    }
    ...
    return map(result, (arr) => f(...(arr as A))) as ValidatedNel<E, B>;
  },
};
