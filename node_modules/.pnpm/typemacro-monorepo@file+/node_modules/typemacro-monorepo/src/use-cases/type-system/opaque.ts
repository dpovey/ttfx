...
export const EmailAddress = opaqueModule<string>("EmailAddress", (s) =>
  /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(s),
)({
  toString: (s) => s,
  domain: (s) => s.split("@")[1],
...
/** Opaque URL */
export const SafeUrl = opaqueModule<string>("SafeUrl", (s) => {
  try {
    new URL(s);
    return true;
  } catch {
  ...
  toString: (s) => s,
  hostname: (s) => new URL(s).hostname,
  pathname: (s) => new URL(s).pathname,
  protocol: (s) => new URL(s).protocol,
});
