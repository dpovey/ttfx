...
function readTsConfig(configPath) {
  ...
  const configFile = ts2.readConfigFile(absolutePath, ts2.sys.readFile);
  if (configFile.error) {
...
function reportDiagnostics(diagnostics) {
  ...
  for (const diagnostic of diagnostics) {
    ...
    if (diagnostic.file && diagnostic.start !== void 0) {
      ...
      const prefix = diagnostic.category === ts2.DiagnosticCategory.Error ? "error" : "warning";
      console.error(
...
function build(options) {
  ...
  const program = ts2.createProgram(config.fileNames, compilerOptions);
  const transformerFactory = macroTransformerFactory(program, {
    verbose: options.verbose
  });
  const emitResult = program.emit(void 0, void 0, void 0, false, {
...
function watch(options) {
  ...
  const host = ts2.createWatchCompilerHost(
    configPath,
    {},
    ts2.sys,
    ts2.createEmitAndSemanticDiagnosticsBuilderProgram,
    (diagnostic) => {
