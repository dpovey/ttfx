 ...
 *
 * Provides:
 * - powerAssert()     — Expression macro: captures every sub-expression on failure
 * - @derive(Arbitrary) — Derive macro: generates random value generators from types
 * - comptimeAssert()  — Expression macro: fail the BUILD if invariant is violated
...
export const powerAssertMacro = defineExpressionMacro({
  name: "powerAssert",
  module: "typemacro/testing",
  description:
  ...
  expand(
    ...
    if (args.length < 1 || args.length > 2) {
      ctx.reportError(
        callExpr,
        "powerAssert expects 1 or 2 arguments: powerAssert(expr) or powerAssert(expr, message)",
      );
      return callExpr;
    }
...
export const comptimeAssertMacro = defineExpressionMacro({
  name: "comptimeAssert",
  module: "typemacro/testing",
  description:
  ...
  expand(
    ...
    if (args.length < 1 || args.length > 2) {
      ctx.reportError(
        callExpr,
        "comptimeAssert expects 1 or 2 arguments: comptimeAssert(expr) or comptimeAssert(expr, message)",
      );
      return callExpr;
    }
    ...
    if (result.kind === "error") {
      ...
      ctx.reportError(
        callExpr,
        `comptimeAssert: cannot evaluate condition at compile time: ${result.message}`,
      );
      return callExpr;
    }
    ...
    if (!isTruthy) {
      ...
      ctx.reportError(
        callExpr,
        `${message}\n  Assertion: comptimeAssert(${sourceText})`,
      );
    }
