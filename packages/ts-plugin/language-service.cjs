...
var TYPESUGAR_PACKAGE_PREFIXES = [
  "typesugar",
  "@typesugar/",
  "typemacro",
  // legacy name
...
function init(modules) {
  ...
  function create(info) {
    ...
    proxy.getSemanticDiagnostics = (fileName) => {
      ...
      return diagnostics.filter((diag) => {
        if (diag.code === UNUSED_IMPORT_CODE) {
          if (diag.start === void 0) return true;
          const node2 = findNodeAtPosition(tsModule, sourceFile, diag.start);
          if (!node2) return true;
          if (isTypesugarImport(tsModule, node2, sourceFile)) {
    ...
    proxy.getCompletionsAtPosition = (fileName, position, options) => {
      ...
      if (deriveContext) {
        const deriveEntries = DERIVE_MACROS.map((macro) => ({
          name: macro.name,
...
function isTypesugarImport(ts, node, sourceFile) {
  let current = node;
  while (current) {
...
function isInDeriveDecorator(ts, node) {
  let current = node;
  while (current) {
    if (ts.isDecorator(current)) {
      const name = getDecoratorName(ts, current);
...
function isExtensionMethodCall(ts, sourceFile, node) {
...
function getExtensionMethodCompletions(ts, sourceFile, node, program) {
  if (!program) return [];
  let propAccess;
  if (ts.isPropertyAccessExpression(node)) {
    propAccess = node;
...
//# sourceMappingURL=language-service.cjs.map